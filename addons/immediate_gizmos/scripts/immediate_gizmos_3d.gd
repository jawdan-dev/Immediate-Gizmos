...
func line_strip(points : Array[Vector3], drawColor : Color = color) -> void:
	ImmediateGizmosInternal.points_3d.append_array(points);
	ImmediateGizmosInternal.end_draw_3d(drawColor, transform);
	
func line_polygon(points : Array[Vector3], drawColor : Color = color) -> void:
	ImmediateGizmosInternal.points_3d.append_array(points);
	if (points.size() <= 0): return;
	ImmediateGizmosInternal.draw_point_3d(points[0])
	ImmediateGizmosInternal.end_draw_3d(drawColor, transform);
	
func line_arc(center : Vector3, axis : Vector3, startPoint : Vector3, radians : float, drawColor : Color = color) -> void:
	ImmediateGizmosInternal.draw_arc_3d(center, axis, startPoint, radians);
	ImmediateGizmosInternal.end_draw_3d(drawColor, transform);

func line_circle(center : Vector3, axis : Vector3, radius : float, drawColor : Color = color) -> void:
...
func line_sphere(center : Vector3, radius : float, drawColor : Color = color) -> void:
...
func line_capsule(center : Vector3, radius : float, height : float, drawColor : Color = color) -> void:
	height -= radius * 2;
	if (height < 0):
...
func line_cuboid(center : Vector3, radius : Vector3, drawColor : Color = color) -> void:
...
func line_cube(center : Vector3, radius : float, drawColor : Color = color) -> void:
